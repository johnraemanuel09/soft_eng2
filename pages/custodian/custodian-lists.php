    ...
    <div class="row mt-4">
      <div class="col-12">
        <div class="card px-4 pb-4">
          ...
          <h5 class="mb-0 pt-4">Custodian Lists</h5>

          <!-- Search bar with clickable magnifying glass -->
          <div class="row mb-3">
              <div class="col-12 d-flex justify-content-center">
                  <div class="input-group w-50 w-md-75 w-sm-100">
                      <input type="text" id="custodianSearch" class="form-control" placeholder="Search custodians...">
          ...
          <div class="table-responsive" id="custodianTableContainer" style="display:none;">
            <table class="table table-flush" id="datatable-search">
              ...
              <tbody>
                <?php
                $listcustodian = mysqli_query($db, "SELECT *, CONCAT(tbl_custodian.firstname, ' ', tbl_custodian.lastname) AS fullname FROM tbl_custodian");
                ...
                <tr>
                  ...
                  <td class="text-sm font-weight-normal">
                    <!-- Edit Button -->
                    <a class="btn btn-link text-info text-gradient px-3 mb-0" href="javascript:;" data-bs-toggle="modal" data-bs-target="#editCustodian<?php echo $id; ?>">
                      <i class="material-icons text-sm me-2">edit</i>Edit
                    </a>
                    <!-- Delete Button -->
                    <a class="btn btn-link text-danger text-gradient px-3 mb-0" href="javascript:;" data-bs-toggle="modal" data-bs-target="#deleteCustodian<?php echo $id; ?>">
                ...
                <!-- Edit Modal -->
                <div class="modal fade" id="editCustodian<?php echo $id; ?>" tabindex="-1" role="dialog" aria-labelledby="editCustodianLabel<?php echo $id; ?>" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="editCustodianLabel<?php echo $id; ?>">Edit Custodian</h5>
                        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <form id="editCustodianForm<?php echo $id; ?>" method="POST">
                        <div class="modal-body">
                          <input type="hidden" name="custodian_id" value="<?php echo $id; ?>">
                          <div class="mb-3">
...
$(document).ready(function() {
    ...
    $('#custodianTableContainer').hide();

    // Press Enter in search bar
    $('#custodianSearch').on('keypress', function(e) {
        if(e.which == 13){ // Enter key
            $('#custodianTableContainer').show();
        }
    });
    ...
    // Live search
    $('#custodianSearch').on('keyup', function() {
    ...
    // AJAX edit forms
    <?php mysqli_data_seek($listcustodian, 0); ?>
    <?php while ($row = mysqli_fetch_array($listcustodian)) { $id = $row['custodian_id']; ?>
    $('#editCustodianForm<?php echo $id; ?>').submit(function(e){
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: 'custodian-edit.php',
            data: $(this).serialize(),
            success: function(response){
                $('#editCustodian<?php echo $id; ?>').modal('hide');
